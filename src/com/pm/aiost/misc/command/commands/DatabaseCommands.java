...
public class DatabaseCommands {
	...
	public static boolean build(CommandSender sender, Command cmd, String label, String[] args) {
	...
	public static boolean query(CommandSender sender, Command cmd, String label, String[] args) {
		if (!CommandUtil.isAdminOrConsole(sender))
			return false;
		if (args.length > 0) {
	...
	public static boolean update(CommandSender sender, Command cmd, String label, String[] args) {
		if (!CommandUtil.isAdminOrConsole(sender))
			return false;
		if (args.length > 0) {
	...
	public static boolean call(CommandSender sender, Command cmd, String label, String[] args) {
		if (!CommandUtil.isAdminOrConsole(sender))
			return false;
		if (args.length > 0) {
			...
			CommandUtil.sendMsg(sender, "Database call complete with result:\n" + Database.resultListToString(result));
			return true;
	...
	public static boolean batch(CommandSender sender, Command cmd, String label, String[] args) {
		if (!CommandUtil.isAdminOrConsole(sender))
			return false;
		if (args.length > 0) {
			int[] result = DatabaseManager.getDatabase().batch(ScriptReader.scriptToBatchList(String.join(" ", args)));
