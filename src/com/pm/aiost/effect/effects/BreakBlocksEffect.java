...
import com.pm.aiost.effect.blueprints.SimpleBlockEffect;
...
public class BreakBlocksEffect extends SimpleBlockEffect {
	...
	public BreakBlocksEffect() {
	}

	public BreakBlocksEffect(byte[] actions, byte condition, int radius) {
	...
	public void runEffect(Block block) {
		WorldEdit.breakBlocks(block, radius, radius, radius);
	}
	...
	public boolean equals(Effect effect) {
		if (!super.equals(effect))
			return false;
		if (((BreakBlocksEffect) effect).radius != radius)
			return false;
		return true;
	}
	...
	public void load(Effect effect) {
		super.load(effect);
		BreakBlocksEffect breakBlocks = (BreakBlocksEffect) effect;
	...
	@Override
	public EffectType<? extends BreakBlocksEffect> getType() {
		return EffectTypes.BREAK_BLOCKS;
	}
