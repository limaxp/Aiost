...
public class ThrowBlockEffect extends Effect {
	...
	public ThrowBlockEffect() {
	}

	public ThrowBlockEffect(byte[] actions, byte condition, float velocityMultiplier, float damage, float knockback) {
	...
	public static void throwItem(LivingEntity entity, ItemStack itemStack, float velocityMultiplier, float damage,
		...
		LaunchBlockEffect.launchBlock(entity, itemStack.getType().createBlockData(), velocityMultiplier, damage,
				knockback, effect);
	}
	...
	public boolean equals(Effect effect) {
		if (!super.equals(effect))
			return false;
		ThrowBlockEffect throwBlock = (ThrowBlockEffect) effect;
		if (throwBlock.velocityMultiplier != velocityMultiplier)
			return false;
		if (throwBlock.damage != damage)
			return false;
		if (throwBlock.knockback != knockback)
			return false;
		if (throwBlock.effect != effect)
			return false;
		return true;
	}
	...
	public void load(Effect effect) {
		super.load(effect);
		ThrowBlockEffect throwBlock = (ThrowBlockEffect) effect;
		if (throwBlock.velocityMultiplier != 0)
			velocityMultiplier = throwBlock.velocityMultiplier;
		if (throwBlock.damage != 0)
			damage = throwBlock.damage;
		if (throwBlock.knockback != 0)
			knockback = throwBlock.knockback;
		if (throwBlock.effect != null)
			effect = throwBlock.effect;
	}
	...
	@Override
	public EffectType<? extends ThrowBlockEffect> getType() {
		return EffectTypes.THROW_BLOCK;
	}
